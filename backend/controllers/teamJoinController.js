// backend/controllers/teamJoinController.js
...
// Cập nhật tin ghép đội (khách, kiểm tra sở hữu bằng số điện thoại)
exports.updatePost = async (req, res) => {
  try {
    ...
    const isOwner =
      ...
      post.contact_phone === updateData.phone_number;

    if (!isOwner) {
      return res
        .status(403)
        .json({ error: 'Bạn không có quyền sửa tin này' });
    }
...
// Cập nhật trạng thái open/closed (khách, kiểm tra sở hữu)
exports.updatePostStatus = async (req, res) => {
  try {
    ...
    const isOwner =
      phone_number && post.contact_phone === phone_number;

    if (!isOwner) {
      return res
        .status(403)
        .json({ error: 'Bạn không có quyền thay đổi trạng thái tin này' });
    }
...
// Xoá tin (khách, soft delete = closed)
exports.deletePost = async (req, res) => {
  try {
    ...
    if (!isOwner) {
      return res
        .status(403)
        .json({ error: 'Bạn không có quyền xóa tin này' });
    }
...
exports.getAllPosts = async (req, res) => {
  try {
    ...
    console.log('Get all team join posts for admin:', {
      page,
      limit,
      status,
      field_type,
    ...
    console.log(
      `Found ${posts.data.length} team join posts for admin`
    );
...
exports.bulkUpdatePostStatus = async (req, res) => {
  try {
    ...
    console.log('Bulk update team join post status:', {
      postIds,
      status,
    });
